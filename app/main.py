...
async def upload_sheet(child_id: int, file: UploadFile = File(...), db: Session = Depends(get_db)):
...
@app.post("/vaccine/")
def create_vaccine(vac: schemas.VaccineCreate, db: Session = Depends(get_db)):
